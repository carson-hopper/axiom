package com.axiommc.api.world;

import java.util.HashMap;
import java.util.Map;

/** All vanilla 1.21 biomes, plus {@link #UNKNOWN} for custom or unrecognised biomes. */
public enum Biome {

    // ── Overworld ─────────────────────────────────────────────────────────────
    THE_VOID,
    PLAINS, SUNFLOWER_PLAINS, SNOWY_PLAINS, ICE_SPIKES,
    DESERT,
    SWAMP, MANGROVE_SWAMP,
    FOREST, FLOWER_FOREST, BIRCH_FOREST, DARK_FOREST, PALE_GARDEN,
    OLD_GROWTH_BIRCH_FOREST, OLD_GROWTH_PINE_TAIGA, OLD_GROWTH_SPRUCE_TAIGA,
    TAIGA, SNOWY_TAIGA,
    SAVANNA, SAVANNA_PLATEAU, WINDSWEPT_HILLS, WINDSWEPT_GRAVELLY_HILLS,
    WINDSWEPT_FOREST, WINDSWEPT_SAVANNA,
    JUNGLE, SPARSE_JUNGLE, BAMBOO_JUNGLE,
    BADLANDS, ERODED_BADLANDS, WOODED_BADLANDS,
    MEADOW, CHERRY_GROVE, GROVE, SNOWY_SLOPES,
    FROZEN_PEAKS, JAGGED_PEAKS, STONY_PEAKS,
    RIVER, FROZEN_RIVER,
    BEACH, SNOWY_BEACH, STONY_SHORE,
    WARM_OCEAN, LUKEWARM_OCEAN, DEEP_LUKEWARM_OCEAN,
    OCEAN, DEEP_OCEAN,
    COLD_OCEAN, DEEP_COLD_OCEAN,
    FROZEN_OCEAN, DEEP_FROZEN_OCEAN,
    MUSHROOM_FIELDS,
    DRIPSTONE_CAVES, LUSH_CAVES, DEEP_DARK,

    // ── Nether ────────────────────────────────────────────────────────────────
    NETHER_WASTES, WARPED_FOREST, CRIMSON_FOREST, SOUL_SAND_VALLEY, BASALT_DELTAS,

    // ── The End ───────────────────────────────────────────────────────────────
    THE_END, END_HIGHLANDS, END_MIDLANDS, SMALL_END_ISLANDS, END_BARRENS,

    // ── Fallback ──────────────────────────────────────────────────────────────
    UNKNOWN;

    // ── Lookup ────────────────────────────────────────────────────────────────

    private static final Map<String, Biome> BY_ID;

    static {
        Map<String, Biome> map = new HashMap<>();
        for (Biome b : values()) {
            if (b != UNKNOWN) {
                map.put("minecraft:" + b.name().toLowerCase(), b);
            }
        }
        BY_ID = Map.copyOf(map);
    }

    /** Namespaced identifier, e.g. {@code "minecraft:plains"}. */
    public String id() {
        return this == UNKNOWN ? "minecraft:unknown" : "minecraft:" + name().toLowerCase();
    }

    /** Returns the enum constant for the given namespaced ID, or {@link #UNKNOWN}. */
    public static Biome of(String id) {
        return BY_ID.getOrDefault(id, UNKNOWN);
    }
}
