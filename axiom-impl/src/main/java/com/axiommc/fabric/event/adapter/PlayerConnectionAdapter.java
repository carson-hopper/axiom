    ...
    public void register(SimpleEventBus eventBus, FabricPlayerProvider playerProvider) {
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            try {
                ...
                FabricPlayer player = playerProvider
                    .player(serverPlayer.getUUID())
                    .orElseGet(() -> new FabricPlayer(serverPlayer));
                String host = server.getLocalIp().isEmpty() ? "localhost" : server.getLocalIp();
                Server serverInfo = new Server("fabric", host, server.getPort());

                eventBus.publish(new PlayerJoinEvent.Pre(player, serverInfo));
                eventBus.publish(new PlayerJoinEvent.Connecting(player));
                eventBus.publish(new PlayerJoinEvent.Post(player, serverInfo));

                // Fire brand event if it was received during config phase
                String brand = ClientBrandTracker.consume(serverPlayer.connection);
                if (brand != null) {
                    PlayerChannelAdapter.onClientBrand(serverPlayer, brand);
                }
            } catch (Exception exception) {
                Axiom.logger().debug("Error firing PlayerJoinEvent", exception);
            }
        });
        ...
        ServerPlayConnectionEvents.DISCONNECT.register((handler, server) -> {
            try {
                ...
                Axiom.logger().debug("Error firing PlayerLeaveEvent", exception);
            }
        });
