...
public class TestPlayerCommand {
    ...
    public void execute(CommandSender sender) {
        sender.sendMessage(
            ChatComponent.text("Usage: /testplayer <title|actionbar|sound>").color(ChatColor.RED));
    }
    ...
    public void title(CommandSender sender) {
        sender.asPlayer().ifPresent(player -> {
            ...
            player.sendMessage(
                ChatComponent.text("Showed title to " + player.name()).color(ChatColor.GREEN));
        });
    }
    ...
    public void actionbar(CommandSender sender) {
        sender.asPlayer().ifPresent(player -> {
            ...
            player.sendMessage(
                ChatComponent.text("Sent actionbar to " + player.name()).color(ChatColor.GREEN));
        });
    }
    ...
    public void sound(CommandSender sender) {
        sender.asPlayer().ifPresent(player -> {
            ...
            player.sendMessage(
                ChatComponent.text("Played sound for " + player.name()).color(ChatColor.GREEN));
        });
    }
}
