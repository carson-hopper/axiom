...
public class GameModeCommand {
    ...
    @Usage("<mode>")
    public void setOwnGameMode(
        Player sender,
        @Arg("mode") GameMode gameMode) {
        sender.gameMode(gameMode);
        sender.sendMessage(
            ChatComponent.textf("Game mode set to {}", gameMode.name().toLowerCase())
    ...
    @Usage("<mode> <player>")
    public void setOtherGameMode(
        CommandSender sender,
        @Arg("mode") GameMode gameMode,
        @Arg("target") Player target) {
        target.gameMode(gameMode);
        sender.sendMessage(ChatComponent.textf(
                "Set {}'s game mode to {}", target.name(), gameMode.name().toLowerCase())
            .color(ChatColor.GREEN));
    }
}
