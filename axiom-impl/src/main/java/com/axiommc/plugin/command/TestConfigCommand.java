...
public class TestConfigCommand {
    ...
    public void execute(CommandSender sender) {
        sender.sendMessage(
            ChatComponent.text("Usage: /testconfig <info|get|set|reload>").color(ChatColor.RED));
    }
    ...
    public void info(CommandSender sender) {
        if (context == null) {
            sender.sendMessage(
                ChatComponent.text("Plugin context not available").color(ChatColor.RED));
            return;
        }
        sender.sendMessage(ChatComponent.text("Plugin Context Info:").color(ChatColor.GOLD));
        sender.sendMessage(
            ChatComponent.text("  Data Folder: " + context.dataFolder().getAbsolutePath())
                .color(ChatColor.GRAY));

        PluginConfig config = context.config();
        sender.sendMessage(
            ChatComponent.text("  Config File: " + config.getClass().getSimpleName())
                .color(ChatColor.GRAY));
    }
    ...
    public void get(
        ...
        if (!config.contains(key)) {
            sender.sendMessage(
                ChatComponent.text("Key '" + key + "' not found").color(ChatColor.RED));
            return;
        }

        String value = config.getString(key, "");
        sender.sendMessage(
            ChatComponent.text("Config[" + key + "] = " + value).color(ChatColor.GREEN));
    }
